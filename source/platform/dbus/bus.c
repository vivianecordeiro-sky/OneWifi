...
static void handle_reg_event_handler(char *event_name,bus_event_sub_action_t action,int32_t interval, bool* autoPublish); 
    ...
    switch (data->data_type) {
        ...
        default:
            wifi_util_info_print(WIFI_BUS, "%s:%d: bus: Invalid data_type:%d.\n",
                 __func__, __LINE__, data->data_type);
        break;
    };
    return str;
}
...
DBusHandlerResult method_message_handler(DBusConnection *conn, DBusMessage *message, void *user_data) 
{
    ...
    if (!strcmp(DBUS_INTERFACE_BASE, interface) && !strcmp("setParameterValues", method_name))
    {
        ...
        if (val->parameterName && val->parameterValue) {
            ...
            result_element = (bus_data_element_t *)hash_map_get(handle->method_callback,val->parameterName);
            if (result_element == NULL) {
                wifi_util_info_print(WIFI_BUS, "result_element is not found hence trying with regex match in the hashmap\n");
                regex_check_for_elements(val->parameterName,reg_ex_match);
                wifi_util_info_print(WIFI_BUS, "reg_ex_match =%s\n",reg_ex_match);
                result_element = (bus_data_element_t *)hash_map_get(handle->method_callback,reg_ex_match);
            }
            if (result_element != NULL) {
                type = convert_ccsp_to_bus_data_type(val->type);
                wifi_util_info_print(WIFI_BUS,"Param name is available in the handler %s and val=%s bus_type=%d bus_registered type=%d\n",val->parameterName,val->parameterValue,type,result_element->data_model_prop.data_format);
                if ((result_element->cb_table.set_handler != NULL) && (type == result_element->data_model_prop.data_format)) {
                    ...
                    if(val->parameterValue != NULL && (data.data_type == bus_data_type_bytes || data.data_type == bus_data_type_string)) {
                        wifi_util_info_print(WIFI_BUS,"This is an string value\n");
                        data.raw_data.bytes = (void*)val->parameterValue;
                    }
                    result = result_element->cb_table.set_handler(val->parameterName,&data, cb_handler);
                }
            }
        }
    ...
    else if (!strcmp(DBUS_INTERFACE_BASE, interface) && !strcmp("getParameterValues", method_name))
    {
        ...
        if (parameterNames)  {
            bus_user_data_t *cb_handler =  (bus_user_data_t *)handle;
            result_element = (bus_data_element_t *)hash_map_get(handle->method_callback,parameterNames);
            if (result_element == NULL) {
                wifi_util_info_print(WIFI_BUS, "result_element is not found hence trying with regex match in the hashmap\n");
            ...
            if (result_element != NULL) {
                ...
                if (result_element->cb_table.get_handler != NULL) {
                    ...
                    result = result_element->cb_table.get_handler(parameterNames,&data,cb_handler);
                    wifi_util_info_print(WIFI_BUS, "%s:%d\n",__func__,__LINE__);
                } 
            }
        } else {
    ...
    else if (!strcmp(DBUS_INTERFACE_BASE, interface) && ((!strcmp("AddTblRow", method_name)) || (!strcmp("DeleteTblRow", method_name))) )
    {
        ...
        if (table_name)  {
            result_element = (bus_data_element_t *)hash_map_get(handle->method_callback,table_name);
            if (result_element == NULL) {
                wifi_util_info_print(WIFI_BUS, "result_element is not found hence trying with regex match in the hashmap\n");
            ...
            if (result_element  != NULL) {
                wifi_util_info_print(WIFI_BUS, "Param name is available in the handler\n");
                if (!strcmp("AddTblRow", method_name) && (result_element->cb_table.table_add_row_handler != NULL)) {
                    wifi_util_info_print(WIFI_BUS,"Call back AddTblRow is present\n");
                    result = result_element->cb_table.table_add_row_handler(table_name,NULL,instanceNumber);
                }
                else if(!strcmp("DeleteTblRow", method_name) &&  (result_element->cb_table.table_remove_row_handler != NULL)) {
                    wifi_util_info_print(WIFI_BUS,"Call back table_remove_row_handler is present\n");
    ...
    else if (!strcmp(DBUS_INTERFACE_BASE, interface) && !strcmp("OneWifiMethod", method_name))
    {
        ...
        if (val->parameterName && val->parameterValue)
        {
            ...
            if (result_element != NULL) {
                type = convert_ccsp_to_bus_data_type(val->type);
                wifi_util_info_print(WIFI_BUS,"Param name is available in the handler %s and val=%s bus_type=%d bus_registered type=%d\n",val->parameterName,val->parameterValue,type,result_element->data_model_prop.data_format);
                if ((result_element->cb_table.method_handler != NULL) && (type == result_element->data_model_prop.data_format)) {
                    ...
                    if(val->parameterValue != NULL && (inParams.data_type == bus_data_type_bytes || inParams.data_type == bus_data_type_string))
                    {
                        wifi_util_info_print(WIFI_BUS,"This is an string value\n");
                        inParams.raw_data.bytes = (void*)val->parameterValue;
                    }
                    result = result_element->cb_table.method_handler(val->parameterName,&inParams,&outParams, cb_handler);
                }
            }
        } else {
            dbus_message_iter_init_append (reply, &iter);
        ...
        if (result == 0)
        {
            ...
            dbus_message_iter_open_container (&array_iter, DBUS_TYPE_STRUCT, NULL,  &struct_iter);
            wifi_util_info_print(WIFI_BUS," %s:%d int_str=%s and type=%d\n",__func__,__LINE__,int_str,type1);
...
bus_error_t send_reg_event_handler(bus_handle_t *handle, char const *event_name,bus_event_sub_action_t action,int32_t interval, bool* autoPublish)
{
    wifi_util_info_print(WIFI_BUS,"%s:Enter %d\n", __func__,__LINE__);
    ...
    message = dbus_message_new_method_call (dst_component_id, dbus_path, DBUS_INTERFACE_BASE, "eventHandler");
    ...
    while(handle != NULL) {
        ...
        if (result_element != NULL) {
            if (result_element->cb_table.event_sub_handler != NULL) {
                ...
                result_element->cb_table.event_sub_handler(event_name, action, interval,autoPublish);
            }
        }
...
static bus_error_t  add_subscription_to_dbus_daemon(bus_handle_t *handle,const char *event_name)
{
    ...
    snprintf (tmp, sizeof(tmp), "type='signal',path='%s',interface='%s',member='%s'",DBUS_PATH_EVENT, DBUS_INTERFACE_EVENT,event_name);
...
bus_error_t append_subscriber_events (bus_handle_t *handle,void *cb, char const *event_name)
{
    ...
    bus_sub_callback_table_t *callback;
    pthread_mutex_lock(&dbus_lock); 
    if ((callback = (bus_sub_callback_table_t *)hash_map_get(handle->subscribe_callback,event_name)) == NULL) {
...
{
    wifi_util_info_print(WIFI_BUS,"%s:Enter %d fullname =%s\n", __func__,__LINE__,element->full_name);
    bus_data_element_t *result_element;
    result_element = (bus_data_element_t *)hash_map_get(handle->method_callback, element->full_name); 
    if (result_element != NULL) {
        pthread_mutex_lock(&dbus_lock);
...
bus_error_t append_method_callback (bus_handle_t *handle, bus_data_element_t *element)
{
    ...
    bus_data_element_t *result_element;
    pthread_mutex_lock(&dbus_lock);
    if ((result_element = (bus_data_element_t *)hash_map_get(handle->method_callback, element->full_name)) == NULL) {
    ...
    while(handle != NULL) { 
        if( handle->u.dbus_handle == connection) {
            ...
            if (callback == NULL) {
                ...
                wifi_util_info_print(WIFI_BUS, "reg_ex_match =%s\n",reg_ex_match);
                callback = (bus_sub_callback_table_t *)hash_map_get(handle->subscribe_callback,reg_ex_match);
            }
            if (callback != NULL) {
                type = convert_ccsp_to_bus_data_type(type);
                wifi_util_info_print(WIFI_BUS, "%s:Enter %d got subscription handler for =%s for handle=%p type=%d\n", __func__,__LINE__,event_name,handle->u.dbus_handle,type);
                convert_dbus_message_to_raw_data(&data,type,val);
                if(val !=NULL && (data.data_type == bus_data_type_bytes || data.data_type == bus_data_type_string))
                {
...
static bus_error_t  add_dbus_handlers(bus_handle_t *handle)
{
    ...
    if (server_thread_start) {
        ...
        if (ret != 0) {
            wifi_util_info_print(WIFI_BUS, "%s:%d failed to create server_loop_thread, "
...
busOpenTelemetryContext* bus_getOpenTelemetryContextFromThreadLocal()
{
    ...
    wifi_util_info_print(WIFI_BUS,"%s:%d\n", __func__, __LINE__);
    busOpenTelemetryContext* ot_ctx = (busOpenTelemetryContext *) pthread_getspecific(_open_telemetry_key);
    if (!ot_ctx)
    {
    ...
    wifi_util_info_print(WIFI_BUS,"%s:%d: Trace parent : %s state : %s\n",
          __func__, __LINE__, ot_ctx->otTraceParent, ot_ctx->otTraceState); 
...
void bus_getOpenTelemetryContext(const char **traceParent, const char **traceState)
{
    ...
    *traceState = &ot_ctx->otTraceState[0];
    wifi_util_info_print(WIFI_BUS,"%s:%d: Trace parent : %s state : %s output Trace parent : %s state : %s\n",
          __func__, __LINE__, ot_ctx->otTraceParent, ot_ctx->otTraceState, *traceParent, *traceState);
}
...
static bus_error_t bus_set_trace_context(bus_handle_t *handle, const char* traceParent, const char* traceState)
{
    if (!handle)
        return bus_error_general;
...
static bus_error_t bus_open(bus_handle_t *handle, char *component_name)
{
    ...
    snprintf(handle->component_name,sizeof(handle->component_name), DBUS_COMP_NAME,component_name);
    ...
    wifi_util_info_print(WIFI_BUS,"%s:%d dbus_bus_request_name  with  comp_name %s:%s\n", __func__, __LINE__,handle->component_name,handle->dbus_path);
    // Get a well known name
    ret = dbus_bus_request_name (p_dbus_handle, handle->component_name, DBUS_NAME_FLAG_REPLACE_EXISTING, &dbus_error);
    if (dbus_error_is_set (&dbus_error)) {
        dbus_error_free(&dbus_error);
        wifi_util_info_print(WIFI_BUS,"%s:%d dbus_bus_request_name failed with  %s:%d\n", __func__, __LINE__,dbus_error.message, ret);
...
static bus_error_t bus_close(bus_handle_t *handle)
{
    ...
    wifi_util_info_print(WIFI_BUS,"%s:%d: bus:bus_close() is successful, handle:%p\n",
         __func__, __LINE__,p_dbus_handle);
...
int handle_namespace_with_cr(
    bus_handle_t *handle,
    const char *subsystem_prefix,
    name_spaceType1_t * name_space,
    int size,bool sub_register
...
)
{
    ...
    if(sub_register) {
        ...
        message = dbus_message_new_method_call (CCSP_CR_COMPONENT_ID,
            CCSP_DBUS_PATH_CR,
    ...
    dbus_message_iter_init_append (message, &iter);
    if(sub_register) {
        wifi_util_info_print(WIFI_BUS,"%s: at %d component_name =%s:%s:%s\n", __func__,__LINE__,handle->component_name,handle->dbus_path,subsystem_prefix);
...
bus_error_t find_destination_path(bus_handle_t *handle,char const *namespace,char **bus_path,char **comp)
{
    ...
    if (strstr(namespace, "eRT.com.cisco") != NULL) {
        wifi_util_info_print(WIFI_BUS,"The substring '%s' was found in the string its psm module .\n", namespace);
        *comp =PSM_COMP_NAME;
    ...
    message = dbus_message_new_method_call (CCSP_CR_COMPONENT_ID, CCSP_DBUS_PATH_CR,CCSP_DBUS_INTERFACE_CR,
...
static bus_error_t bus_set(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    message = dbus_message_new_method_call (dst_component_id, dbus_path, DBUS_INTERFACE_BASE, "setParameterValues");
...
static bus_error_t bus_data_get(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    message = dbus_message_new_method_call (dst_component_id, dbus_path, DBUS_INTERFACE_BASE, "getParameterValues");
    if (!message )
    {
        wifi_util_info_print(WIFI_BUS, "No memory\n");
        return bus_error_out_of_resources;
    ...
    wifi_util_info_print(WIFI_BUS,"error %s:%d type=%d\n",__func__,__LINE__,type);
     if( (data->data_type == bus_data_type_bytes || data->data_type == bus_data_type_string) && val->parameterValue) {
...
static bus_error_t bus_raw_event_publish(bus_handle_t *handle, char *name, void *data, unsigned int size)
{
    DBusMessage *message;
    VERIFY_NULL_WITH_RC(name);
    VERIFY_NULL_WITH_RC(data);
...
{ 
    ...
    for (index = 0; index < num_of_element; index++) {
        ...
        element_namespace[index].dataType = convert_bus_to_ccsp_data_type(data_element[index].data_model_prop.data_format);
   }
...
{ 
    ...
    element_namespace = (name_spaceType1_t *) calloc(num_of_element, sizeof(name_spaceType1_t));
    if (element_namespace == NULL) {
        wifi_util_info_print(WIFI_BUS,"%s:%d bus: bus_reg_data_elements() calloc for namespace is failed\n",
            __func__, __LINE__);
        return bus_error_out_of_resources;
    } 
    for (index = 0; index < num_of_element; index++) {
        ...
        dbus_dataElement.data_model_prop.data_format = data_element[index].data_model_prop.data_format;
        ...
        dbus_dataElement.cb_table.table_add_row_handler = data_element[index].cb_table.table_add_row_handler;
        dbus_dataElement.cb_table.table_remove_row_handler = data_element[index].cb_table.table_remove_row_handler;
        dbus_dataElement.cb_table.event_sub_handler = data_element[index].cb_table.event_sub_handler;
    ...
    switch(input_bus_data) {
        ...
        case BUS_METHOD_SET_GET:
            ...
            if(strcmp(event,"Device.WiFi.AP.STA.GetAssocRequest") == 0) {
                ...
                wifi_util_info_print(WIFI_BUS,"dbus_path = %s and component = %s\n",dbus_path, dst_component_id);
                message = dbus_message_new_method_call (dst_component_id, dbus_path, DBUS_INTERFACE_BASE, "OneWifiMethod");
                if (!message )
                {
                    wifi_util_info_print(WIFI_BUS,"No memory\n");
                    return bus_error_out_of_resources;
                ...
                if ((output_data->data_type == bus_data_type_bytes || output_data->data_type == bus_data_type_string) && val->parameterValue) {
                    output_data->raw_data.bytes = (void *)calloc((output_data->raw_data_len + 1), sizeof(char));
    ...
    void *userData, int timeout)
{
    ...
    if (rc == bus_error_success) {
        ...
        if (rc == bus_error_success) {
            ...
            wifi_util_info_print(WIFI_BUS, "Successfully appended subscription event to global array\n");
        }
    }
    add_dbus_handlers(handle);
    return rc; 
    ...
    int num_sub, int timeout)
{
    ...
    for (int index = 0; index < num_sub; index++) {  
        rc = append_subscriber_events(handle,l_sub_info_map[index].handler,(char *)l_sub_info_map[index].event_name);  
        if (rc == bus_error_success) {
            wifi_util_info_print(WIFI_BUS, "Successfully appended subscription event to global array\n");
            // Handle event handler in bus registration table
            send_reg_event_handler(handle,(char *)l_sub_info_map[index].event_name,bus_event_action_subscribe,0, false);
        }
    }
    add_dbus_handlers(handle);
    return rc;
...
{
#define busError_String(E, S) case E: s = S; break;
    ...
    switch (e)
    {
        ...
        busError_String(bus_error_invalid_response_from_destination, "invalid response from destination");
