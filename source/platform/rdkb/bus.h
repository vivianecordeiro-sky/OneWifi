...
typedef struct bus_filter {
    union {
        rbusFilter_t rbus_event_filter;
    } u;
} bus_filter_t;
...
typedef void (*bus_sub_async_resp_handler_t)(bus_handle_t *handle, bus_event_sub_t *subscription,
    bus_error_t error);
...
typedef bus_error_t (
    *wifi_bus_data_get_t)(bus_handle_t *handle, char const *name, raw_data_t *data);
