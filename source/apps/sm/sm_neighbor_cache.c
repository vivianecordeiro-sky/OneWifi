    ...
    dpp_neighbor_record_list_t *result, survey_type_t survey_type)
{
    ...
    wifi_util_dbg_print(WIFI_SM, "%s:%d: Fetched neighbor %s sample on %s channel %u SSID %s chanwidth %d\n",
        __func__, __LINE__, survey_type_to_str(survey_type),
        radio_index_to_radio_type_str(radio_index), hal->ap_Channel, hal->ap_SSID, entry->chanwidth);
